...
from zjbs_file_client import CompressMethod, download_directory, download_file, upload, upload_directory
...
async def test_download_file(file_server_file: Path):
    with SpooledTemporaryFile() as tmp_file:
        await download_file("/test_download_file/test.txt", tmp_file)
        tmp_file.seek(0)
...
async def test_download_directory(file_server_file: Path):
    with TemporaryDirectory() as tmp_dir:
        await download_directory("/test_download_directory", tmp_dir)
...
async def test_upload(temp_file: SpooledTemporaryFile) -> None:
    ...
    try:
        await upload("/", temp_file, "test.txt", mkdir=False, allow_overwrite=False)
...
async def test_upload_directory(temp_directory: Path) -> None:
    ...
    try:
        await upload_directory("/", temp_directory, CompressMethod.tgz, mkdir=False)
